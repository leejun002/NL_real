...
def pipeline_and_save_noise(
    ...
    print("Normalized: mean=", noise_norm.mean(), ", std=", noise_norm.std())
...
def train(config):
    """
    1) pipeline_and_save_noise(...) 호출하여 txt -> noise_data.mat 생성
    2) 그 결과를 Read_data -> Make_dataset -> DataLoader
    3) U-Net 학습
    """
    import os
    from torch.utils.data import DataLoader
...
def batch_predict(config):
    ...
    for i in range(n_files):
        ...
        bg_2d = np.zeros_like(row_2d)
        for r in range(rank_bg):
            if r < len(s):
                bg_2d += np.outer(U[:, r], s[r] * Vt[r, :])
        bg_1d = bg_2d.flatten()  # (1600,)
