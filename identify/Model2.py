...
# 텐서 크기 조정 함수
def match_tensor_sizes(tensor1, tensor2, dim=2):
    """Match the sizes of two tensors along the given dimension."""
    if tensor1.shape[dim] != tensor2.shape[dim]:
        diff = tensor1.shape[dim] - tensor2.shape[dim]
        pad = (0, abs(diff)) if diff < 0 else (abs(diff), 0)
...
class UNet_CA(nn.Module, ABC):
    ...
    def forward(self, x):
        ...
        # Decoding + Concatenate path
        d5 = self.Up5(x5, target_size=x4.shape[2:])
        x4, d5 = match_tensor_sizes(x4, d5)
        d5 = self.Up_conv5(torch.cat((x4, d5), dim=1))

        d4 = self.Up4(d5, target_size=x3.shape[2:])
        x3, d4 = match_tensor_sizes(x3, d4)
        d4 = self.Up_conv4(torch.cat((x3, d4), dim=1))

        d3 = self.Up3(d4, target_size=x2.shape[2:])
        x2, d3 = match_tensor_sizes(x2, d3)
        d3 = self.Up_conv3(torch.cat((x2, d3), dim=1))

        d2 = self.Up2(d3, target_size=x1.shape[2:])
        x1, d2 = match_tensor_sizes(x1, d2)
