...
from Model2 import UNet_CA, UNet, UNet_CA6, UNet_CA7
...
def predict(config):
    ...
    model = eval("{}(1,1)".format(config.model_name))
    save_model_path = save_model_dir(config)
    model_file = os.path.join(save_model_path, f"{config.global_step}.pt")
    state = torch.load(model_file, map_location='cuda' if torch.cuda.is_available() else 'cpu')
    model.load_state_dict({k.replace('module.', ''): v for k, v in state['model'].items()})
    print(f'Successfully loaded the model saved at global step = {state["global_step"]}')
    model.eval()
    ...
    for file in filenames:
        if os.path.splitext(file)[1] == '.txt':
            filepath = os.path.join(config.predict_root, file)
            output_filepath = os.path.join(config.predict_root, f"dn_{file}")
            
            # 데이터 로드
            data = np.loadtxt(filepath, delimiter='\t').astype(float)
            wave, x = data[:, 0], data[:, 1]

            # 입력 데이터 전처리 (DCT 변환 및 크기 조정)
            coe_dct = dct(x, norm='ortho')
            coe_dct = coe_dct.reshape(1, 1, -1)  # 3D 텐서로 변환
            coe_dct = torch.from_numpy(coe_dct).float()

            if torch.cuda.is_available():
                coe_dct = coe_dct.cuda()

            # 예측 수행
            with torch.no_grad():
                yt = model(coe_dct).detach().cpu().numpy()
            ...
            # 결과 저장
            result = np.column_stack((wave, denoised))
            np.savetxt(output_filepath, result, delimiter='\t', fmt='%.6f', header='Wave\tDenoised', comments='')
            print(f"Denoised data saved to {output_filepath}")

            # 결과 시각화
            plt.figure()
            plt.plot(wave, x, label='Noisy Spectrum')
            plt.plot(wave, denoised, label='Denoised Spectrum')
            plt.legend()
            plt.xlabel('Wave')
            plt.ylabel('Intensity')
            plt.title(f'Denoising Result - {file}')
            plt.show()
